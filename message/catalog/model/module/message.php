...
class ModelModuleMessage extends Model {
	...
	public function getMessages($page = 1)
	{
		...
		$sql = "SELECT msg.id,msg.SenderId,msg.ReceiverId,msg.SendTime,msg.ReadFlag,msgt.messageText FROM " . DB_PREFIX . "message msg INNER JOIN " . DB_PREFIX . "messageText msgt ON msg.MessageTextId=msgt.Id WHERE msg.ReceiverId=".(int)$this->customer->getId()." order by msg.ReceiverId asc,msg.id desc limit " . $limit . ",20";
	...
	{
		$query = $this->db->query("SELECT COUNT(*) AS total FROM " . DB_PREFIX . "message where ReceiverId=".(int)$this->customer->getId());
	...
	{
		$query = $this->db->query("SELECT COUNT(*) AS total FROM " . DB_PREFIX . "message where ReadFlag = 0 and ReceiverId=".(int)$this->customer->getId());
